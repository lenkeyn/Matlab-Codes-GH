function sData = placecellComparison3_Morestim(sData,type)
...
for i=1:1:sData.imdata.nROIs 
    ...
    if sum(any(sData.imdata.MaoOptoMoreProt_dff{1,1}.PlaceCells(:,1)==i)) || sum(any(sData.imdata.MaoOptoMoreProt_dff{1,2}.PlaceCells(:,1)==i)) || sum(any(sData.imdata.MaoOptoMoreProt_dff{1,3}.PlaceCells(:,1)==i)) || sum(any(sData.imdata.MaoOptoMoreProt_dff{1,5}.PlaceCells(:,1)==i)) 
...
if type == 0 
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'MeanActivitySortedComparisonMoreProt-dff');
    savePath = strcat(sData.sessionInfo.savePath,'\Imaging\MeanActivitySortedComparisonMoreProt-dff');
else
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'MeanActivitySortedComparisonMoreProt-deconv');
    savePath = strcat(sData.sessionInfo.savePath,'\Imaging\MeanActivitySortedComparisonMoreProt-deconv');
end
...
Max(:,1) = max(sData.imdata.MaoOptoMoreProt_dff{1, 1}.PosTuning(AllPlaceCells,:),[],2);
Max(:,2) = max(sData.imdata.MaoOptoMoreProt_dff{1, 2}.PosTuning(AllPlaceCells,:),[],2);
Max(:,3) = max(sData.imdata.MaoOptoMoreProt_dff{1, 3}.PosTuning(AllPlaceCells,:),[],2);
Max(:,4) = max(sData.imdata.MaoOptoMoreProt_dff{1, 5}.PosTuning(AllPlaceCells,:),[],2);
MaxAct = max(Max,[],2);


%laser-off control dff (protocol #1 )
% nomrlize data
HeatMapData = sData.imdata.MaoOptoMoreProt_dff{1, 1}.PosTuning(AllPlaceCells,:); % sData.imdata.MaoPC.OptoOff.PosTuningOrig(sData.imdata.MaoPC.OptoOff.PlaceCells,:)
NormHeatMapData = HeatMapData./MaxAct;
% sorting
MaxData = max(NormHeatMapData, [], 2);
MaxDataBin = NaN(ROINu,2);
...
SortingOrder = sortrows(MaxDataBin, 1); % second column is the sorted ROI order. plot in these order
SortedData1 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.
MeanProtocol1 = nanmean(SortedData1,1);
%PLOT FIGURE
figure('Color','white'); 
imagesc(Xaxis,Yaxis,SortedData1) %(1:number of bins;1:number of trials)
c = colorbar; colormap(jet); caxis([0 1]);
...
%protocol #2 
HeatMapData =  sData.imdata.MaoOptoMoreProt_dff{1, 2}.PosTuning(AllPlaceCells,:);
NormHeatMapData = HeatMapData./MaxAct;
SortedData2 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
MeanProtocol2 = nanmean(SortedData2,1);
%PLOT FIGURE
figure('Color','white'); 
imagesc(Xaxis,Yaxis,SortedData2) %(1:number of bins;1:number of trials) 
c = colorbar; colormap(jet); caxis([0 1]); hold on;
...
%protocol #3 
HeatMapData =  sData.imdata.MaoOptoMoreProt_dff{1, 3}.PosTuning(AllPlaceCells,:);
NormHeatMapData = HeatMapData./MaxAct;
SortedData3 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
%PLOT FIGURE
MeanProtocol3 = nanmean(SortedData3,1);
figure('Color','white'); 
imagesc(Xaxis,Yaxis,SortedData3) %(1:number of bins;1:number of trials) 
c = colorbar; colormap(jet); caxis([0 1]); hold on;
c.Label.String = 'dFF'; c.Label.FontSize = 11; c.TickDirection = 'out'; 
...
%protocol #5 
HeatMapData =  sData.imdata.MaoOptoMoreProt_dff{1, 5}.PosTuning(AllPlaceCells,:);
NormHeatMapData = HeatMapData./MaxAct;
SortedData5 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
MeanProtocol5 = nanmean(SortedData5,1);
%PLOT FIGURE
figure('Color','white'); 
imagesc(Xaxis,Yaxis,SortedData5) %(1:number of bins;1:number of trials) 
c = colorbar; colormap(jet); caxis([0 1]); hold on;
c.Label.String = 'dFF'; c.Label.FontSize = 11; c.TickDirection = 'out'; 
line([sData.behavior.opto.optoStimStart sData.behavior.opto.optoStimStart],[0 ROINu],'Color','white','LineStyle','-','LineWidth',2); hold on;    
line([sData.behavior.opto.optoStimEnd sData.behavior.opto.optoStimEnd],[0 ROINu],'Color','white','LineStyle','-','LineWidth',2);    
...
FileName = strcat(sData.sessionInfo.fileID,'-Prot5-dff-SortedMeanAct'); 
savefig(fullfile(savePath,FileName));
saveas(gcf,(fullfile(savePath,[FileName '.jpg'])));
%}
...
hold on
plot(Xaxis,mean(SortedData5),'LineStyle','-','LineWidth',1)
hold on
...
xlabel('Position on Wheel (cm)'); ax = gca; ax.TickDir = 'out'; xticks([0,50,100,150]);
ylabel('Position tuning');
legend('Laser-off', 'Laser-on 3 mW', 'Laser-on 2 mW', 'Laser-after','Location','south');
...
if type == 0
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.AllPlaceCells = AllPlaceCells;
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.SortingOrder = SortingOrder;
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.AllPC.MeanProt1 = MeanProtocol1;
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.AllPC.MeanProt2 = MeanProtocol2;
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.AllPC.MeanProt3 = MeanProtocol3;
    sData.imdata.MaoPC_Sorting_dFF_MoreOpto.AllPC.MeanProt5 = MeanProtocol5;
    else
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.AllPlaceCells = AllPlaceCells;
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.SortingOrder = SortingOrder;
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.AllPC.Prot1 = MeanProtocol1;
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.AllPC.Prot2 = MeanProtocol2;
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.AllPC.Prot3 = MeanProtocol3;    
    sData.imdata.MaoPC_Sorting_deconv_MoreOpto.AllPC.Prot5 = MeanProtocol5;
end
%}
