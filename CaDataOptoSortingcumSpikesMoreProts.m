function sData  = CaDataOptoSortingcumSpikesMoreProts(sData,FigVisible) 

%%% Set parameters:
datatype = 2; % datatype can be 0 = CumSpikes or 1 = CumSpikesolved, 2= cumSpiking dataset
...
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'RoiActCumSpikes-OptoMoreProt');
SavePath = strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt');
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt'),'ROIActCumSpikes');
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt'),'MeanROIActCumSpikes');
...
for i = 1:1:nOptoProt
    for roi = 1:1:nROIs
        sData.imdata.binned.optoMoreProts.ROIsActCumSpikes_Protocol{i,roi} = NaN(nTrials,nBins); % each Xth ROI's activity is the Xth a matrix in this cell array
        sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{i} = NaN(nROIs,nBins);
    end
end
...
for roi = 1:1:sData.imdata.nROIs
    ...
    elseif datatype == 1
        data = sData.imdata.binned.RoiCumSpikesolved{1,roi};
    elseif datatype == 2
        data = sData.imdata.binned.RoicumSpikes{1,roi};    
    end
    ...
    for i = 1:1:nOptoProt 
        ...
        sData.imdata.binned.optoMoreProts.ROIsActCumSpikes_Protocol{i,roi}(k,:) = data(k,:);
        sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{i}(roi,:) = nanmean(data(k,:),1);
    end
   ...
   %%% PLOT
    ...
    for j=1:1:nOptoProt
        ...
        imagesc(Xaxis,1:nTrials,sData.imdata.binned.optoMoreProts.ROIsActCumSpikes_Protocol{j,roi}); 
        if j>1
        ...
        colormap(colors); caxis([Min Cmax]); %Cmin/10 for CumSpikes  % -Cmax/10 for spikerate
        ...
        if Ymax < max(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(roi,:)) % looking for Ymax value for the next plot
            Ymax = max(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(roi,:));
        end
    end
    % mean act plots
    for j=1:1:nOptoProt
        subplot(3,nOptoProt,j+2*nOptoProt); 
        plot(Xaxis,sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(roi,:)); 
        if j>1
    ...
    suptitleN(strcat(sData.sessionInfo.fileID,'-ROI#',num2str(roi),'-CumSpikes'));    
    FileName = strcat(sprintf('ROI-%d',roi),'-',text,'-optoMoreProt'); 
    savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt\ROIActCumSpikes'),FileName));
    saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt\ROIActCumSpikes'),[FileName '.jpg'])));
    
    
    %%% PLOT mean ROI activity in different protocols in one plot
    figure('Color','white','visible',FigVisible)
    for j=1:1:nOptoProt
        plot(Xaxis,smoothdata(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(roi,:),'gaussian',5),'LineWidth',1.5); hold on;
    end
    ...
    legend(Protocols,'Location','North','FontSize',12); 
    title(strcat(sData.sessionInfo.fileID,'-ROI#',num2str(roi),'-CumSpikes'));
    FileName = strcat(sprintf('ROI-%d',roi),'-',text,'-optoMoreProtMean-CumSpikes-G5'); 
    savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt\MeanROIActCumSpikes'),FileName));
    saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActCumSpikes-OptoMoreProt\MeanROIActCumSpikes'),[FileName '.jpg'])));
...
Ymax = 0;
for j=1:1:nOptoProt
    plot(Xaxis,nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(:,:),1),'LineWidth',1.5); hold on; %smoothdata(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanAct_Protocol{1,j}(:,:),1)
    if max(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(:,:),1),[],2) > Ymax
        Ymax = max(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActCumSpikes_Protocol{1,j}(:,:),1),[],2);
    end
end
...
FileName = strcat(sData.sessionInfo.fileID,'-optoMoreProtMeanAllROIsCumSpikes'); 
