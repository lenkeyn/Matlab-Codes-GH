function sData = placecellComparison2(sData,type,GaussFilter)
...
for i=1:1:sData.imdata.nROIs 
   ...
   %}
    %{
    if sum(any(sData.imdata.MaoPC_dff.LightOff.PlaceCells(:,1)==i)) || sum(any(sData.imdata.MaoPC_dff.LightOn.PlaceCells(:,1)==i)) || sum(any(sData.imdata.MaoPC_dff.LightAfter.PlaceCells(:,1)==i)) %|| sum(any(sData.imdata.MaoOpto_dff{1,4}.PlaceCells(:,1)==i))  
        AllPlaceCellsPre(i,1) = i;  
    end
     %}
end
...
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'MeanActivitySortedComparison');
savePath = strcat(sData.sessionInfo.savePath,'\Imaging\MeanActivitySortedComparison');
...
%%% light off , light on, light after trials
...
if type ~= 0
    ...
    GaussianSmoothedLightOff = NaN(length(AllPlaceCells),sData.behavior.meta.nBins); %length(sData.imdata.binned.LightOffTrials)
    GaussianSmoothedLightOn = NaN(length(AllPlaceCells),sData.behavior.meta.nBins);
    GaussianSmoothedLightAfter = NaN(length(AllPlaceCells),sData.behavior.meta.nBins);
    for i = 1:1:ROINu
        j = AllPlaceCells(i);
        GaussianSmoothedLightOff(i,:) = smoothdata(sData.imdata.MaoPC.LightOff.PosTuningOrig(j,:),'gaussian',GaussFilter);
        GaussianSmoothedLightOn(i,:) = smoothdata(sData.imdata.MaoPC.LightOn.PosTuningOrig(j,:),'gaussian',GaussFilter);
        GaussianSmoothedLightAfter(i,:) = smoothdata(sData.imdata.MaoPC.LightAfter.PosTuningOrig(j,:),'gaussian',GaussFilter);
    end
    
    MaxDeconv = NaN(length(AllPlaceCells),3);
    MaxDeconv(:,1) = max(GaussianSmoothedLightOff,[],2);
    MaxDeconv(:,2) = max(GaussianSmoothedLightOn,[],2);
    MaxDeconv(:,3) = max(GaussianSmoothedLightAfter,[],2);
    MaxDeconvAct = max(MaxDeconv,[],2);
    
    % plot laser-off deconv smoothed
    HeatMapData = GaussianSmoothedLightOff;
...
%light-on deconv
if type ~= 0
    HeatMapData = GaussianSmoothedLightOn;
    %Max = max(HeatMapData,[],2);
    NormHeatMapData = HeatMapData./MaxDeconvAct;
    SortedData2 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
    %PLOT FIGURE
...
%light-after deconv
if type ~= 0
    HeatMapData = GaussianSmoothedLightAfter;
    %Max = max(HeatMapData,[],2);
    NormHeatMapData = HeatMapData./MaxDeconvAct;
    SortedData3 = NormHeatMapData(SortingOrder(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
    %PLOT FIGURE
...
%mean activity of all ROIs light-off-on-after in one plot
figure('Color','white','Position',[100 100 500 400]);
plot(Xaxis,MeanActLaserOff,'LineStyle','-','LineWidth',1) %Xaxis,CADATA.Opto.ROIsMeanAct_LightOffTrials(roi,1:BinNu)
hold on
...
% nomrlize data
HeatMapData = sData.imdata.MaoOpto_dff{1, 1}.PosTuning(AllPlaceCells,:); % sData.imdata.MaoPC.LightOff.PosTuningOrig(sData.imdata.MaoPC.LightOff.PlaceCells,:)
NormHeatMapData = HeatMapData./MaxAct;
% sorting
MaxData = max(NormHeatMapData, [], 2);
MaxDataBin = NaN(ROINu,2);
...
plot(Xaxis,mean(SortedData1),'LineStyle','-','LineWidth',1) %Xaxis,CADATA.Opto.ROIsMeanAct_LightOffTrials(roi,1:BinNu)
hold on
...
xlabel('Position on Wheel (cm)'); ax = gca; ax.TickDir = 'out'; xticks([0,50,100,150]);
ylabel('Position tuning');
%legend('Laser-off', 'Laser-on #1', 'Laser-on #2','Location','south');
