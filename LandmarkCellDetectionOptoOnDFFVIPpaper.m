    ...
    for i = 1:1:sData.imdata.nROIs % comparing peak positions (for a ROI) if the distance of peaks equals to distance between landmarks 
        if PCdff.Criteria123Passed(i) > 0 % if the cell was a place cell in that protocol check if it was a landmark cell
            ...
            if PeakAmplitudeLocation >= VelcroStart1 &&  PeakAmplitudeLocation <= VelcroEnd1 % Velcro1 is the main peak, the second peak should be at least this large
                MinPeakDetectionSecondPeak = PeakAmplitude/4;
            ...
            elseif PeakAmplitudeLocation >= HGSStart2 &&  PeakAmplitudeLocation <= HGSEnd2 % HGS2 is the main peak
                MinPeakDetectionSecondPeak = PeakAmplitude/4; % because dFF tail of the first peak often still there
...
for k = 1:1:3 % run in each protocol
    ...
    for i = 1:1:sData.imdata.nROIs % search for place cells with one peak 
        if PCdff.Criteria123Passed(i) > 0 && ~any(LandmarkDetectionDFF.LandmarkCellinOptoOfforOptoOn==i)  % if the cell is a place cell but not a landmark cell
    ...
    for m = 1:1:3
        if m == 1
            sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF3.OptoOff = PlaceCellsWithOnePF;
        elseif m == 2
            sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF3.OptoOn = PlaceCellsWithOnePF;
        elseif m == 3
            sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF3.OptoAfter = PlaceCellsWithOnePF;
        end
    end
    
end

sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF3.PlaceCellsWithOnePFinAllProt = intersect(intersect(sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoOff.PlaceCellsWithOnePFList,sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoOn.PlaceCellsWithOnePFList),sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoAfter.PlaceCellsWithOnePFList);
sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF3.PlaceCellsWithOnePFinAnyProt = union(union(sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoOff.PlaceCellsWithOnePFList,sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoOn.PlaceCellsWithOnePFList),sData.imdata.MaoPC_Opto_dff.PlaceCellsWithOnePF.OptoAfter.PlaceCellsWithOnePFList);
