function sData  = CaDataAlignToLicks(sData,FigVisible) 
...
%create struct
sData.imdata.AlignedLick = struct;
...
LickStartSamplesPre2 = LickStartSamplesPre;
for i = 2:1:length(LickStartSamplesPre)
    if LickStartSamplesPre(i) - LickStartSamplesPre(i-1) < DataShownBeforeStimSamples % there must be a 60 samples lick-free period
        LickStartSamplesPre2(i) = NaN;
    end
end
LickStartSamples = LickStartSamplesPre2(~isnan(LickStartSamplesPre2));
...
xlabel('time (ms)'); ax = gca; ax.TickDir = 'out'; 
ylabel('activity aligned to lick-start (dF/F)');
title(strcat(sData.sessionInfo.fileID,' Mean lick-aligned activity all ROIs norm'));
FileName = strcat(sData.sessionInfo.fileID,'-Mean-lick-aligned-act-all-ROIs norm'); 
savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\AlignDffToLicks\ROIAct'),FileName));
saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\AlignDffToLicks\ROIAct'),[FileName '.jpg'])));
...
xlabel('time (ms)'); ax = gca; ax.TickDir = 'out'; 
ylabel('activity aligned to lick-start (dF/F)');
title(strcat(sData.sessionInfo.fileID,' Mean lick-aligned normalized activity all events'));
FileName = strcat(sData.sessionInfo.fileID,'-Mean-lick-aligned-norm-act-all-events'); 
savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\AlignDffToLicks\ROIAct'),FileName));
saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\AlignDffToLicks\ROIAct'),[FileName '.jpg'])));
