function sData  = CaDataOptoSortingdFFsDataMoreProts2(sData,datatype,FigVisible,ProtocolNames) 

%%% Set parameters:
%datatype = 0; % datatype can be 0 = dFF or 1 = Deconvolved, 2= cumSpiking dataset
% Protocols = string({sData.stimProtocols.protocol});
% ProtocolNames = string({'none','stim 3 mW/mm2'}); %'stim 10 mW','stim 6mW','stim 3 mW ','stim 1 mW','stim 14-84 cm','stim 86-156 cm'
...
nOptoProt = length(sData.stimProtocols); % number of protocols
...
for roi = 1:1:sData.imdata.nROIs
    ...
    for i = 1:1:nOptoProt 
        k = find(sData.behavior.optoMoreProts.OptoStimProtTrialsReal == i); % 0 means failed trials (discard)
   ...
   %%% PLOT
    ...
    for j=1:1:nOptoProt
        ...
        plotdata = sData.imdata.binned.optoMoreProts.ROIsActdFF_Protocol{j,roi};
        plotdata(plotdata<0)=0; % set negative values to zero for illustration
        imagesc(Xaxis,1:nTrials,plotdata); 
        if j>1
        ...
        title(ProtocolNames(j),'FontSize',12);
    ...
    legend(ProtocolNames,'Location','south','FontSize',12); 
