function sData = placeCellMaosData2_MatesDataconversion(sData) %InOutRatio,ReliabiliyIndex,ROIs
...
datatype = 0; % use dff
savePathMain = 'C:\MATLAB\SAVE\forNoraC';

%%%PARAMATERS TO SET:
% datatype = 0; 0: use dff data, 1: use deconvolved data, -1: use dff, where slow transients are removed 
...
BinNu = sData.behavior.trialMatrices.meta.binNumber;
TRNu = sData.behavior.trialMatrices.meta.nAllTrials;
...
dataset = struct;
if datatype == 0 
    mkdir(strcat(savePathMain,'\',sData.sessionInfo.sessionID),'MaoPlaceCells-dFF');
    SavePath = strcat(savePathMain,'\',sData.sessionInfo.sessionID,'\','MaoPlaceCells-dFF');
    dataset = sData.imdata.binnedRoisDff;
    Mao.datatype = 'dFF';
%{
elseif datatype == -1
...
for i = 1:1:ROINu
    Mao.PosTuning(i,1:BinNu) = nanmean(dataset(1:end,1:end,i),1);    % i ROI,      
end
...
for i = 1:1:ROINu
    if any(~isnan(PotPlaceFieldStartBin2(i,:))) % if no potential place field, jump to next roi
        ...
        for j = 1:1:TRNu-1
            TrialData = dataset(j,:,i); % put each trial data into a container
            if any(isnan(TrialData)) % after the end  of recording there are NaNs. Go to next trial
               continue %break 
            end
...
if datatype == 0 
    ...
    fn = 'dff';
    dataForSortPC = Mao.PosTuning(Mao.PlaceCells,:);
    dataForSort = Mao.PosTuning;
    %{
elseif datatype == -1 
...
for j = 1:1:length(Mao.PlaceCells)
    ...
    fname = strcat(sData.sessionInfo.sessionID,fn,'-roi',num2str(i));
    savefig(fullfile(SavePath,fname));
    saveas(gcf,(fullfile(SavePath,[fname '.png'])));
    close all;
end
...
%PLOT FIGURE SMOOTHED
figure('Color','white'); % smoothdata(SortedDataMax,2,'gaussian',5)
imagesc(1:BinSize:BinSize*BinNu,1:PCNu,smoothdata(SortedDataMax,2,'gaussian',5)) %(1:number of bins;1:number of trials)
c = colorbar;
colormap(jet);
c.Label.String = fn;
c.Label.FontSize = 11;
c.TickDirection = 'out'; 
xlabel('Position on Wheel (cm)'); ax = gca; ax.TickDir = 'out'; xticklabels = 0:50:250; %cannot set the axis to show 157 as the end%
Xticks = linspace(1, 250, numel(xticklabels)); set(gca, 'XTick', Xticks, 'XTickLabel', xticklabels)
ylabel('ROIs'); yticklabels = 0:10:ROINu; yticks = linspace(1, ROINu, numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
title(strcat(sData.sessionInfo.sessionID,'Gaussian-smooting')); 
savefig(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-GaussSm-',fn,'.fig')));
saveas(gcf,(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-GaussSm-',fn,'.png'))));

%PLOT FIGURE NOT NORM
figure('Color','white'); 
imagesc(1:1:BinSize:BinSize*BinNu,1:PCNu,smoothdata(SortedData)) %(1:number of bins;1:number of trials)
c = colorbar;
colormap(jet);
c.Label.String = fn;
c.Label.FontSize = 11;
...
ylabel('ROIs'); yticklabels = 0:10:ROINu; yticks = linspace(1, ROINu, numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
title(strcat(sData.sessionInfo.sessionID,'not-normalized')); 
savefig(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-not-normalized.fig')));
saveas(gcf,(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-not-normalized.png'))));

%PLOT FIGURE ZSCORED SMoothed
figure('Color','white'); 
imagesc(1:BinSize:BinSize*BinNu,1:PCNu,zscore(smoothdata(SortedData,2,'gaussian',5)')') %(1:number of bins;1:number of trials)
c = colorbar;
colormap(jet);
...
ylabel('ROIs'); yticklabels = 0:10:ROINu; yticks = linspace(1, ROINu, numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
title(strcat(sData.sessionInfo.sessionID,'GaussSm-Zscored')); 
savefig(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-GaussSm-Zscored.fig')));
saveas(gcf,(fullfile(SavePath,strcat(sData.sessionInfo.sessionID,'PCMeanAct-GaussSm-Zscored.png'))));
...
save(fullfile(strcat(savePathMain,'\',sData.sessionInfo.sessionID),strcat(sData.sessionInfo.sessionID,'.mat')),'sData');
