function sData  = CaDataOptoSortingDeconvMoreProts(sData,FigVisible) 

%%% Set parameters:
datatype = 1; % datatype can be 0 = Deconv or 1 = Deconvolved, 2= cumSpiking dataset
...
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'RoiActDeconv-OptoMoreProt');
SavePath = strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt');
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt'),'ROIActDeconv');
mkdir(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt'),'MeanROIActDeconv');
...
for i = 1:1:nOptoProt
    for roi = 1:1:nROIs
        sData.imdata.binned.optoMoreProts.ROIsActDeconv_Protocol{i,roi} = NaN(nTrials,nBins); % each Xth ROI's activity is the Xth a matrix in this cell array
        sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{i} = NaN(nROIs,nBins);
    end
end
...
for roi = 1:1:sData.imdata.nROIs
    ...
    if datatype == 0
        data = sData.imdata.binned.RoiDeconv{1,roi};
    elseif datatype == 1
    ...
    for i = 1:1:nOptoProt 
        ...
        sData.imdata.binned.optoMoreProts.ROIsActDeconv_Protocol{i,roi}(k,:) = data(k,:);
        sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{i}(roi,:) = nanmean(data(k,:),1);
    end
   ...
   %%% PLOT
    ...
    for j=1:1:nOptoProt
        ...
        imagesc(Xaxis,1:nTrials,sData.imdata.binned.optoMoreProts.ROIsActDeconv_Protocol{j,roi}); 
        if j>1
        ...
        colormap(colors); caxis([Min Cmax]); %Cmin/10 for Deconv  % -Cmax/10 for spikerate
        ...
        if Ymax < max(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(roi,:)) % looking for Ymax value for the next plot
            Ymax = max(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(roi,:));
        end
    end
    % mean act plots
    for j=1:1:nOptoProt
        subplot(3,nOptoProt,j+2*nOptoProt); 
        plot(Xaxis,sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(roi,:)); 
        if j>1
    ...
    suptitleN(strcat(sData.sessionInfo.fileID,'-ROI#',num2str(roi),'-Deconv'));    
    FileName = strcat(sprintf('ROI-%d',roi),'-',text,'-optoMoreProt'); 
    savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt\ROIActDeconv'),FileName));
    saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt\ROIActDeconv'),[FileName '.jpg'])));
    
    
    %%% PLOT mean ROI activity in different protocols in one plot
    figure('Color','white','visible',FigVisible)
    for j=1:1:nOptoProt
        plot(Xaxis,smoothdata(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(roi,:),'gaussian',5),'LineWidth',1.5); hold on;
    end
    ...
    title(strcat(sData.sessionInfo.fileID,'-ROI#',num2str(roi),'-Deconv'));
    FileName = strcat(sprintf('ROI-%d',roi),'-',text,'-optoMoreProtMean-Deconv-G5'); 
    savefig(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt\MeanROIActDeconv'),FileName));
    saveas(gcf,(fullfile(strcat(sData.sessionInfo.savePath,'\Imaging\RoiActDeconv-OptoMoreProt\MeanROIActDeconv'),[FileName '.jpg'])));
...
Ymax = 0;
for j=1:1:nOptoProt
    plot(Xaxis,nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(:,:),1),'LineWidth',1.5); hold on; %smoothdata(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanAct_Protocol{1,j}(:,:),1)
    if max(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(:,:),1),[],2) > Ymax
        Ymax = max(nanmean(sData.imdata.binned.optoMoreProts.ROIsMeanActDeconv_Protocol{1,j}(:,:),1),[],2);
    end
end
...
FileName = strcat(sData.sessionInfo.fileID,'-optoMoreProtMeanAllROIsDeconv'); 
