function sData = placecellComparison4_1stim(sData,type,GaussFilter)
...
if type == 0 
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'MeanActivitySortedComparison-dff-new');
    savePath = strcat(sData.sessionInfo.savePath,'\Imaging\MeanActivitySortedComparison-dff-new');
else
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'MeanActivitySortedComparison-deconv-new');
    savePath = strcat(sData.sessionInfo.savePath,'\Imaging\MeanActivitySortedComparison-deconv-new');
end


%separate odd and even Opto-off trials (only opto-off place cells)
%clearvars -except sData
nROIs = length(sData.imdata.MaoPC_Opto_dff.OptoOff.PlaceCells);
TrialNo = length(sData.behavior.opto.OptoOffTrialsIndices);
x = 1:TrialNo;
odv = x(rem(x,2)==1); %odv = 1:2:TrialNo;
evn = x(rem(x,2)==0); %evn = 2:2:TrialNo;
optoOffTrialsOdd = sData.behavior.opto.OptoOffTrialsIndices(odv);
optoOffTrialsEven = sData.behavior.opto.OptoOffTrialsIndices(evn);
ROIsMeanAct_OptoOffTrialsOdd(nROIs,sData.behavior.meta.nBins) = NaN;
ROIsMeanAct_OptoOffTrialsEven(nROIs,sData.behavior.meta.nBins) = NaN;
for i = 1:1:nROIs
    j = sData.imdata.MaoPC_Opto_dff.OptoOff.PlaceCells(i); 
    ROIsMeanAct_OptoOffTrialsOdd(i,1:sData.behavior.meta.nBins) = nanmean(sData.imdata.binned.RoidFF{1,j}(optoOffTrialsOdd,1:sData.behavior.meta.nBins),1);
    ROIsMeanAct_OptoOffTrialsEven(i,1:sData.behavior.meta.nBins) = nanmean(sData.imdata.binned.RoidFF{1,j}(optoOffTrialsEven,1:sData.behavior.meta.nBins),1);
end
sData.imdata.binned.ROIsMeanAct_OptoOffTrialsOdd_optoOffPC = ROIsMeanAct_OptoOffTrialsOdd;
sData.imdata.binned.ROIsMeanAct_OptoOffTrialsEven_optoOffPC = ROIsMeanAct_OptoOffTrialsEven;

%corr
M = corr(ROIsMeanAct_OptoOffTrialsOdd,ROIsMeanAct_OptoOffTrialsEven);
figure
imagesc(M)
mean(max(M))
sData.imdata.binned.OptoOffTrialsEvenVsOdd_PopVectCorr = M;
sData.imdata.binned.OptoOffTrialsEvenVsOdd_PopVectCorrMaxMean = mean(max(M));
%clearvars -except sData
...
% plot even and odd opto-off
if type == 0
    Xaxis = sData.behavior.meta.binSize:sData.behavior.meta.binSize:sData.behavior.meta.binSize*sData.behavior.meta.nBins;
    YAxis = 1:1:nROIs; 
    % odd
    HeatMapData = ROIsMeanAct_OptoOffTrialsOdd(1:nROIs,:);
        % sorting
    MaxData2 = max(HeatMapData, [], 2);
    MaxDataBin2 = NaN(nROIs,2);
    MaxDataBin2(:,2) = (1:nROIs);
    for i = 1:1:nROIs
        MaxDataBin2(i,1) = find(HeatMapData(i,:) == MaxData2(i));
    end
    SortingOrder2 = sortrows(MaxDataBin2, 1); % second column is the sorted ROI order. plot in these order
    SortedData2 = HeatMapData(SortingOrder2(:,2),:); % new matrix containing sorted data.
    
    
    %Max = max(HeatMapData,[],2);
    NormHeatMapData = HeatMapData./MaxData2;
    SortedData2 = NormHeatMapData(SortingOrder2(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
    %PLOT FIGURE
    figure('Color','white'); 
    imagesc(Xaxis,YAxis,SortedData2) %(1:number of bins;1:number of trials) 
    c = colorbar; colormap(jet); caxis([0 1]); hold on;
    c.Label.String = 'dFF'; c.Label.FontSize = 11; c.TickDirection = 'out'; 
    line([sData.behavior.opto.optoStimStart sData.behavior.opto.optoStimStart],[0 nROIs],'Color','white','LineStyle','-','LineWidth',2); hold on;    
    line([sData.behavior.opto.optoStimEnd sData.behavior.opto.optoStimEnd],[0 nROIs],'Color','white','LineStyle','-','LineWidth',2);    
    xlabel('Position on Wheel (cm)'); ax = gca; ax.TickDir = 'out'; xticks([0,25,50,75,100,125,150]);
    ylabel('ROIs'); yticklabels = 0:10:nROIs; yticks = linspace(1, nROIs, numel(yticklabels));
    set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
    title('ROI activity sorted based on peak activity - Laser-off-odd'); 
    FileName = strcat(sData.sessionInfo.fileID,'-LaserOff1-odd-dff-SortedMeanAct'); 
    savefig(fullfile(savePath,FileName));
    saveas(gcf,(fullfile(savePath,[FileName '.jpg'])));
    
    % even
    HeatMapData = ROIsMeanAct_OptoOffTrialsEven(1:nROIs,:);
    %Max = max(HeatMapData,[],2);
    NormHeatMapData = HeatMapData./MaxData2;
    SortedData2 = NormHeatMapData(SortingOrder2(:,2),:); % new matrix containing sorted data.  % NormHeatMapData
    %PLOT FIGURE
    ...
    ylabel('ROIs'); yticklabels = 0:10:nROIs; yticks = linspace(1, nROIs, numel(yticklabels));
    set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
    title('ROI activity sorted based on peak activity - Laser-off-even'); 
    FileName = strcat(sData.sessionInfo.fileID,'-LaserOff2-even-dff-SortedMeanAct'); 
    savefig(fullfile(savePath,FileName));
    saveas(gcf,(fullfile(savePath,[FileName '.jpg'])));
    ...
    %M=corr(ROIsMeanAct_OptoOffTrialsEven(1:nROIs,:)',ROIsMeanAct_OptoOffTrialsEven(1:nROIs,:)');
end
