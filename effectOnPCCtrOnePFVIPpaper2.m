function sData = effectOnPCCtrOnePFVIPpaper2(sData)
...
% generate datastruct
sData.effectPCinCtrOnePFFinal2 = struct;
sData.effectPCinCtrOnePFFinal2.PFPeakAmplatCtrPeakPosOffOnAfter = NaN(nROIs,3); % peak amplitude at that position which is the peak in Opto-off, calculated from pos tuning curve
sData.effectPCinCtrOnePFFinal2.meanActOutPFCtrPos = NaN(nROIs,3); % mean Ca activity outside of the PF, PF is determined in OPto-off
sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutCtrPos = NaN(nROIs,3); 
sData.effectPCinCtrOnePFFinal2.PFStartPosBinOffOnAfter = NaN(nROIs,3);  % Starting bin of the PF (where pos tuning curve first goes above 1/3 of the peak compared to baseline), can be different in different protocols
sData.effectPCinCtrOnePFFinal2.PFLengthBinOffOnAfter = NaN(nROIs,3); % length of PF (number of bins where pos tuning curve are above 1/3 of the peak compared to baseline)
sData.effectPCinCtrOnePFFinal2.PFPeakAmplOffOnAfter = NaN(nROIs,3); % peak amplitude of PF , calculated from pos tuning curve
sData.effectPCinCtrOnePFFinal2.PFPeakPosBinOffOnAfter = NaN(nROIs,3); 
sData.effectPCinCtrOnePFFinal2.meanActOutPFNewPos= NaN(nROIs,3); % mean Ca activity outside of the PF (discard beginning of the track data, where no stimulation in opto-on trials)
sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutNewPos = NaN(nROIs,3); % ratio of peak activity inside/ mean activity outside the PF
...
for i = 1:1:nROIs
    ...
    sData.effectPCinCtrOnePFFinal2.PFPeakAmplatCtrPeakPosOffOnAfter(i,1:3) = [PeakAmplOff PeakAmplOnAtCtrPeakPos PeakAmplAfterAtCtrPeakPos];
    sData.effectPCinCtrOnePFFinal2.meanActOutPFCtrPos(i,1:3) = [meanActOutPFOff meanActOutPFCtrPosOn meanActOutPFCtrPosAfter];
    sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutCtrPos(i,1:3) = [peakActInPFMeanOutRatioOff RATIOpeakActInPFCtrPosMeanOutOn RATIOpeakActInPFCtrPosMeanOutAfter]; 
    % PF is allowed to shift +-5 bins in OPto-off and after protocols,
    % check values in these new PFs
    sData.effectPCinCtrOnePFFinal2.PFStartPosBinOffOnAfter(i,1:3) = [PeakStartPosOff PeakStartPosOn PeakStartPosAfter]; 
    sData.effectPCinCtrOnePFFinal2.PFLengthBinOffOnAfter(i,1:3) = [PeakLengthOff PeakLengthOn PeakLengthAfter];
    sData.effectPCinCtrOnePFFinal2.PFPeakAmplOffOnAfter(i,1:3) = [PeakAmplOff PeakAmplOn PeakAmplAfter];
    sData.effectPCinCtrOnePFFinal2.PFPeakPosBinOffOnAfter(i,1:3) = [PeakPosOff PeakPosOn PeakPosAfter]; 
    sData.effectPCinCtrOnePFFinal2.meanActOutPFNewPos(i,1:3) = [meanActOutPFOff meanActOutPFNewPosOn meanActOutPFNewPosAfter]; 
    sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutNewPos(i,1:3) = [peakActInPFMeanOutRatioOff RATIOpeakActInPFNewPosMeanOutOn RATIOpeakActInPFNewPosMeanOutAfter]; 
end

sData.effectPCinCtrOnePFFinal2.note = 'Place cells detected in opto-off protocol';
sData.effectPCinCtrOnePFFinal2.onlyValues = struct;
sData.effectPCinCtrOnePFFinal2.onlyValues.PFPeakAmplatCtrPeakPosOffOnAfter = sData.effectPCinCtrOnePFFinal2.PFPeakAmplatCtrPeakPosOffOnAfter(find(~isnan(sData.effectPCinCtrOnePFFinal2.PFPeakAmplatCtrPeakPosOffOnAfter(:,1))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.meanActOutPFCtrPos = sData.effectPCinCtrOnePFFinal2.meanActOutPFCtrPos(find(~isnan(sData.effectPCinCtrOnePFFinal2.meanActOutPFCtrPos(:,1))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.RatioPeakActInPFMeanOutCtrPos = sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutCtrPos(find(~isnan(sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutCtrPos(:,1))),1:3);

sData.effectPCinCtrOnePFFinal2.onlyValues.PFStartPosBinOffOnAfter = sData.effectPCinCtrOnePFFinal2.PFStartPosBinOffOnAfter(find(~isnan(sData.effectPCinCtrOnePFFinal2.PFStartPosBinOffOnAfter(:,2))),1:3); % value should not be NaN in Opto-On trials
sData.effectPCinCtrOnePFFinal2.onlyValues.PFLengthBinOffOnAfter = sData.effectPCinCtrOnePFFinal2.PFLengthBinOffOnAfter(find(~isnan(sData.effectPCinCtrOnePFFinal2.PFLengthBinOffOnAfter(:,2))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.PFPeakAmplOffOnAfter = sData.effectPCinCtrOnePFFinal2.PFPeakAmplOffOnAfter(find(~isnan(sData.effectPCinCtrOnePFFinal2.PFPeakAmplOffOnAfter(:,2))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.PFPeakPosBinOffOnAfter = sData.effectPCinCtrOnePFFinal2.PFPeakPosBinOffOnAfter(find(~isnan(sData.effectPCinCtrOnePFFinal2.PFPeakPosBinOffOnAfter(:,2))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.meanActOutPFNewPos = sData.effectPCinCtrOnePFFinal2.meanActOutPFNewPos(find(~isnan(sData.effectPCinCtrOnePFFinal2.meanActOutPFNewPos(:,2))),1:3);
sData.effectPCinCtrOnePFFinal2.onlyValues.RatioPeakActInPFMeanOutNewPos = sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutNewPos(find(~isnan(sData.effectPCinCtrOnePFFinal2.RatioPeakActInPFMeanOutNewPos(:,2))),1:3);
