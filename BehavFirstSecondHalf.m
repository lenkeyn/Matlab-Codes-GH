function sData = BehavFirstSecondHalf(sData,LapsTested) % LapsTested = 30; % how many laps take to analyse in the beginning and end

sData.behavior.FirstSecondHalf = struct;
%savePath = 'C:\MATLAB\SAVE';
mkdir(strcat(sData.sessionInfo.savePath,'\Behavior'),'FirstSecondHalf');
savePath = strcat(sData.sessionInfo.savePath,'\Behavior\FirstSecondHalf');
...
FileName = strcat(sData.sessionInfo.fileID,'SpeedFirstSecondHalf-lap-',num2str(FirstStartLap),'-',num2str(FirstEndLap),'-',num2str(SecondStartLap),'-',num2str(SecondEndLap));
savefig(fullfile(savePath,FileName));
saveas(gcf,(fullfile(savePath,[FileName '.jpg'])));     
