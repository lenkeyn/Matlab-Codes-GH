function sData = placecellMaosDataOptoOnMoreProt2(sData,datatype,InOutRatio,ReliabiliyIndex,Protocol,FigVisible)
...
MaoOpto.activityTreshold = activityTreshold;  
MaoOpto.InOutRatio = InOutRatio;
MaoOpto.ReliabilityIndex = ReliabiliyIndex;
...
Trials = find(sData.behavior.optoMoreProts.OptoStimProtTrialsReal==Protocol);
...
%{
% until 2023.01.19 this was the code:
for i = 1:1:ROINu
...
for i = 1:1:ROINu 
    for j = 1:1:20 % if ROI has more than one potential place fields, check all of them
        ...
        ActInPlaceField(i,j) = mean(ActAllTrials(i,PotPlaceFieldPos(i,j):(PotPlaceFieldPos(i,j)+PotPlaceFieldLength(i,j)-1)));  
        ActOutPlaceField(i,j) = mean(ActAllTrials(i,PotPlaceFieldPos(i,j)+PotPlaceFieldLength(i,j)+1:(PotPlaceFieldPos(i,j)+BinNu)-1));
        end
    end
end
...
for i = 1:1:ROINu
    if any(~isnan(PotPlaceFieldPos2(i,:))) % if no potential place field, jump to next roi
        ...
        PotBins = PotPlaceFieldPos2(i,:); % look for potential PF positions after CRITERIA 2 is tested
        PotBins = PotBins(~isnan(PotBins));
        PotLength = PotPlaceFieldLength2(i,:);
        PotLength = PotLength(~isnan(PotLength));
        PotPlaceFieldNu = sum(~isnan(PotPlaceFieldPos2(i,:)));
        ...
        PFLength = PotPlaceFieldLength2(i,PotPlaceFieldPos2(i,:)==PFStart);
        ...
        for m = 1:1:TrialNu
            ...
            if MaxSmoothed < (mean(TrialData) + 1*(std(TrialData))) % the peak has to be larger than to mean+noise to be consideres as a peak, otherwise it is considered as noise
                continue
            end                
...
if datatype == 0
    sData.imdata.MaoOptoMoreProt_dff{Protocol} = struct;
    sData.imdata.MaoOptoMoreProt_dff{Protocol} = MaoOpto;
    mkdir(makedir,strcat('\Prot',num2str(Protocol),'-dff'));
    SavePath = strcat(makedir,'\Prot',num2str(Protocol),'-dff');
    fn = 'dff';
elseif datatype == 1
    sData.imdata.MaoOptoMoreProt_deconv{Protocol} = struct;
    sData.imdata.MaoOptoMoreProt_deconv{Protocol} = MaoOpto;
...
for j = 1:1:length(MaoOpto.PlaceCells)
    ...
    for k = 1:1:size(MaoOpto.PlaceFieldStartBin,2)
        if ~isnan(MaoOpto.PlaceFieldStartBin(i,k))
            if MaoOpto.PlaceFieldStartBin(i,k) + MaoOpto.PlaceFieldBinLength(i,k)-1 < BinNu +1
                rectangle('Position',[MaoOpto.PlaceFieldStartBin(i,k)*BinSize Ymax/500 (MaoOpto.PlaceFieldBinLength(i,k)-1)*BinSize Ymax],'FaceColor',[1 0.95 1],'EdgeColor','none'); hold on;
                line([MaoOpto.PlaceFieldStartBin(i,k)*BinSize MaoOpto.PlaceFieldStartBin(i,k)*BinSize],[0 Ymax],'Color',[1 0.3 0.2],'LineStyle','--','LineWidth',1); hold on; line([(MaoOpto.PlaceFieldStartBin(i,k)+MaoOpto.PlaceFieldBinLength(i,k)-1)*BinSize (MaoOpto.PlaceFieldStartBin(i,k)+MaoOpto.PlaceFieldBinLength(i,k)-1)*BinSize],[0 Ymax],'Color',[1 0.3 0.2],'LineStyle','--','LineWidth',1); hold on;
            else
                rectangle('Position',[MaoOpto.PlaceFieldStartBin(i,k)*BinSize Ymax/500 (BinNu-MaoOpto.PlaceFieldStartBin(i,k))*BinSize Ymax],'FaceColor',[1 0.95 1],'EdgeColor','none'); hold on;
                rectangle('Position',[BinSize Ymax/500 (MaoOpto.PlaceFieldStartBin(i,k)+MaoOpto.PlaceFieldBinLength(i,k)-BinNu-1)*BinSize Ymax],'FaceColor',[1 0.95 1],'EdgeColor','none'); hold on;
                line([MaoOpto.PlaceFieldStartBin(i,k)*BinSize MaoOpto.PlaceFieldStartBin(i,k)*BinSize],[0 Ymax],'Color',[1 0.3 0.2],'LineStyle','--','LineWidth',1); hold on; 
                line([(MaoOpto.PlaceFieldStartBin(i,k)+MaoOpto.PlaceFieldBinLength(i,k)-BinNu-1)*BinSize (MaoOpto.PlaceFieldStartBin(i,k)+MaoOpto.PlaceFieldBinLength(i,k)-BinNu-1)*BinSize],[0 Ymax],'Color',[1 0.3 0.2],'LineStyle','--','LineWidth',1); hold on;
            end
        end
    end
    %%%
