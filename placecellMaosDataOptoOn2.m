function sData = placecellMaosDataOptoOn2(sData,datatype,FigVisible)
...
MaoOpto.Reliability = ReliabiliyIndex;
...
MaoOpto.OptoOff.PosTuningOrig = NaN(ROINu,BinNu);
MaoOpto.OptoOn.PosTuningOrig = NaN(ROINu,BinNu);
MaoOpto.OptoAfter.PosTuningOrig = NaN(ROINu,BinNu);
for i = 1:1:ROINu
    for j = 1:1:BinNu
...
if datatype == 1
    ...
    MaoOpto.OptoOn.PosTuningSmoothed = GaussianSmoothedOptoOn;
    MaoOpto.OptoOff.PosTuningSmoothed = GaussianSmoothedOptoOff;
    MaoOpto.OptoAfter.PosTuningSmoothed = GaussianSmoothedOptoAfter;
else
...
if datatype == 0
    sData.imdata.MaoPC_Opto_dff = struct;
    sData.imdata.MaoPC_Opto_dff = MaoOpto;
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'PlaceCellOpto-dff');
    savePathPre = strcat(sData.sessionInfo.savePath,'\Imaging\PlaceCellOpto-dff');
elseif datatype == 1
    sData.imdata.MaoPC_Opto_deconv = struct;
    sData.imdata.MaoPC_Opto_deconv = MaoOpto;
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'PlaceCellOpto-deconv');
    savePathPre = strcat(sData.sessionInfo.savePath,'\Imaging\PlaceCellOpto-deconv');
elseif datatype == 2
    sData.imdata.MaoPC_Opto_spikerate = struct;
    sData.imdata.MaoPC_Opto_spikerate = MaoOpto;
    mkdir(strcat(sData.sessionInfo.savePath,'\Imaging'),'PlaceCellOpto-spikerate');
    savePathPre = strcat(sData.sessionInfo.savePath,'\Imaging\PlaceCellOpto-spikerate');
end
